...
void SparseMMatrixM::read_len_hdf5(
    ...
    const int i,      // linear or quadratic
  ...
  // row lengths -------------------------------------------------------------
  hid_t dataset = H5Dopen(file_id, (std::string("LEN0") + std::to_string(i)).c_str(),H5P_DEFAULT);
  hid_t status = H5Dread(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, len_row);
  assert(status == 0);
  // matrix row off entry lengths --------------------------------------------
  dataset = H5Dopen(file_id, (std::string("OFFLEN0") + std::to_string(i)).c_str(), H5P_DEFAULT);
...
void SparseMMatrixM::read_pos_hdf5(
  ...
  hid_t dataset = H5Dopen(file_id, (std::string("POS0") + std::to_string(i)).c_str(), H5P_DEFAULT);
  hid_t status = H5Dread(dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, pos_row);
  assert(status == 0);
  dataset = H5Dopen(file_id, (std::string("VAL0") + std::to_string(i)).c_str(), H5P_DEFAULT);
...
int SparseMMatrixM::read_dim_hdf5(
  ...
  hid_t file_id = H5Fopen(namefile.c_str(), H5F_ACC_RDWR, H5P_DEFAULT);
  // linear ------------------------------------------------
  hid_t dataset = H5Dopen(file_id, (std::string("DIM0") + std::to_string(i)).c_str(), H5P_DEFAULT);
