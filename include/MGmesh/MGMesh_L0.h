...
//   double _mesh2_data[10];  //< Only for coupled mesh
    ...
    MGGeomElBase& _GeomEl;    ///< Geometric element class
  ...
  int* _min_nd[3];  ///< Node map indices subdomain-level min offset (_node_map [0]quad [1]lin [2]const)
  int* _max_nd[3];  ///< Node map indices subdomain-level max offset (_node_map [0]quad [1]lin [2]const)
  ...
  double* _yplus;
  std::vector<double> _Oxyz_pts; //  reference system at point (normal,stream,binormal)
  int _allow_Oxyz=0;
  ...
  MGMesh(const ParallelObjectM& comm, MGUtils& mgutils, MGGeomElBase& geomel);
  // ------------------------------------------------------------------------------------
...
//   virtual double _set_read_rot(double theta,double Coxyz[]){_theta=theta;*_Coxyz=*Coxyz;} 
  ...
  ///@}
  void active_Oxyz(int on){_allow_Oxyz=on;}
  // ======================================================================================
  // GET RETURN FUNCTIONS
  // ======================================================================================
  void get_el_nodes(const int el_nnodes, const int bdry, const int Level, const int iel, double xx[]) const;
  ///< Get element nodes
  ...
  void get_el_nod_conn(const int bdry, const int Level, const int iel, int el_conn[], double xx[], int iproc) const;
  /// Get element connectivity and node coordinates from exact position in global map
  void get_el_nod_conn(const int bdry, const int iel, int el_conn[], double xx[]) const;
  /// Get element connectivity and node coordinates from exact position in global map
  void get_el_nod_conn_global(const int bdry, const int Level, const int iel, int el_conn[], double xx[]) const;
...
//   void set_mesh_attribute(int pos, std::string ss){ _label[pos]=ss;}
 ...
 private:
  ...
  void print_conn_quad_hf5( const int Level);    
  ///@}
