...
#include "MGFemus_L0.h"
#include "MGGeom_L1_Map.h"
...
class FEMUS: public FEMUS0 {
    // PROTECTED VARIABLES ================================================================
protected:
    // data communication (defined in Constructor)
    // FEMus classes
    MGUtils &      _mg_utils;                      // Parameters and files (hdf5,no mpi, no med)
    MGGeomMap   &  _mg_geomap;                     // Element type (hdf5 ?,no mpi,no med)
    MGGeomElBase  &   _mg_geomel;
    
    MGFEMap &      _mg_femap;                      // Finite Element Map (hdf5 ?,no mpi,no med)

    // processors -------------------------------------------------------------------------
//          MGFemusInit*  _start;                         // start function
    std::unique_ptr<MGFemusInit>  _start;
    ...
    int _i_in;        ///< initial time iteration
    int  _n_steps;    ///< final time iteration
    int _i_step;      ///< actual time iteration
    int _i_print_step;  ///< print every
    double _time;           ///< actual time
    double _dt;             ///< step time

    // mesh ----------------------------------------------------------------------------
    MeshExtended* _mg_mesh;                       // FEMus-mesh
    bool _MgMeshInitialized;

    std::unique_ptr<EquationsMap> eqnMap_;

    std::unique_ptr<MGFEbase> dfe_q_;
    std::unique_ptr<MGFEbase> dfe_l_;
    std::unique_ptr<MGFEbase> dfe_k_;
    std::unique_ptr<MGFEbase> face_dfe_q_;
    std::unique_ptr<MGFEbase> face_dfe_l_;
    std::unique_ptr<MGFEbase> face_dfe_k_;
    std::unique_ptr<MGFEbase> dfe_rt_;
...
public:
  ...
  // ===================================================================
    ...
    FEMUS();
    // FEMUS(MPI_Comm comm);
    // FEMUS(int &argc, char** & argv, MGUtils& mgutils);
    
    

    
FEMUS(int &argc, char** & argv,   MGUtils& mgutils,   MGGeomMap & mggeomel,   MGFEMap& mgfemap);
    // Init structures
    //! This function initialize FEMUS class object
    void init_femus(int &argc, char** & argv);
    ...
    void init_fem(/*std::unique_ptr<MGGeomElBase>  mggeomel,*/ MGFEMap& mgfemap);
    //! This function initialize the finite element and functions
    void init_fem();
    //! This function initialize the map of equations to solve
    void init_equation_system(int n_data_points = 0, int n_data_cell = 0);
    ...
    void set_time(
        ...
        const int  n_steps,   ///< final time iteration -> t_in+ n_steps
        const int t_step,     ///< actual time iteration
        const int print_step, ///< print every
        double time,          ///< actual time
        double dt             ///< step time

    ){_i_in= t_in; _n_steps= n_steps; _i_step= t_step; _i_print_step=print_step; _time=time; _dt=dt;}

    // check variable and processors -----------------------------------------
    ...
    inline int IsVelFieldActive() {
        int a = 0;
        if (stoi(_mg_utils._dict_config["MG_NavierStokes"]) != 0 ||
                stoi(_mg_utils._dict_config["MG_FluidStructure"]) != 0)
            a = 1;
        return a;
    }
    ...
    void solve_steady(
        ...
        const int& eq_max = 40  ///< eq max to solve -> enum  FIELDS (equations_conf.h)
    );
    ...
    //     const double zMax);
    void setCtrlDomain(const std::vector<std::vector<double>> coord);
    
        //! This function sets the controllable domain
    // void setControlledDomain(
    //     const double xMin, const double xMax, const double yMin, const double yMax, const double zMin,
    //     const double zMax);
    void setControlledDomain(const std::vector<std::vector<double>> coord);

        //! This function sets the lift domain
    // void setLiftDomain(
    //     const double xMin, const double xMax, const double yMin, const double yMax, const double zMin,
    //     const double zMax);
    void setLiftDomain(const std::vector<std::vector<double>> coord);
    // ================================================================================================
...
//! non linear= solve_non_linear_onestep + update_non_linear_onestep
    double solve_non_linear_onestep(
        double     toll,        ///< tolerance
        int        max_iter,    ///< max non linear iterations
        const int& eq_min,      ///< eq min to solve -> enum  FIELDS (equations_conf.h) (in)
        const int& eq_max       ///< eq max to solve -> enum  FIELDS (equations_conf.h) (in)
    );
    //! This function updates solutions  for transient problems.
    //! non linear= solve_non_linear_onestep + update_non_linear_onestep
    void update_non_linear_onestep(
        double     toll,        ///< tolerance
        int        max_iter,    ///< max non linear iterations
        const int& eq_min,      ///< eq min to solve -> enum  FIELDS (equations_conf.h) (in)
        const int& eq_max      ///< eq max to solve -> enum  FIELDS (equations_conf.h) (in)
...
public:
    ...
    MEDCoupling::MEDCouplingFieldDouble * getNodeMapMed() {
        return _NodeMap.back();
    };
