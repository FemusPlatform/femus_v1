...
public:
   ...
   * Destructor.
   */
  virtual ~MGFEC() = default;

  /* \returns The number of shape functions associated with this finite element of type \p fe_t.
...
//   typedef unsigned int (double []) (const ElemType,  const Order,  const unsigned int);
// 
//   /**
...
//    */
  ...
  static int * n_dofs_at_node_function(const unsigned int dim,  const FEType & fe_t);
...
//    *
//    * \todo For consistency with other MGFEC routines, this function
...
// 
   ...
   */
  static bool on_reference_element(const double & p, const ElemType t, const double eps=TOLERANCE);
  /**
  ...
  // /   static Real shape(const FEType & fe_t, int extra_order,   const Elem * elem,    const unsigned int i,
//                     const Point & p);
  static double shape(const unsigned int dim,  const FEType & fe_t, const ElemType t, const unsigned int i,  const double  p[]);
...
// 
   ...
   * extra_order parameter.
   */
  //   template<typename OutputType> static void shape(const unsigned int dim,  const FEType & fe_t,
//                     const Elem * elem, const unsigned int i,    const Point & p, OutputType & phi);
//   template<typename OutputType>   static void shape(const FEType & fe_t,        const Elem * elem,
//                     const unsigned int i, const Point & p,   OutputType & phi);
//   template<typename OutputType>   static void shape(const FEType & fe_t,                    int extra_order,
//                     const Elem * elem,     const unsigned int i,            const Point & p,   OutputType & phi);
//   template<typename OutputType>   static void shapes(const unsigned int dim,  const FEType & fe_t,     const Elem * elem,
//                      const unsigned int i, const std::vector<Point> & p,    std::vector<OutputType> & phi,  const bool add_p_level = true);
  static void shape(const unsigned int dim, const FEType & fe_t, const ElemType t,  const unsigned int i, const double p[],  double  phi[]);
...
//    * be used instead.
//    *
//    * \todo To be consistent with the other non-deprecated MGFEC
...
// 
  ...
  static double *  shape_function(const unsigned int dim,  const FEType & fe_t,    const ElemType t);
  ...
  //  * \deprecated Call the version of this function taking an Elem* instead.
  //  */
  //   static Real shape_deriv (const unsigned int dim,   const FEType & fe_t,  const Elem *elem,
//                            const unsigned int i,  const unsigned int j,   const Point & p);
//   static Real shape_deriv(const FEType & fe_t,   const Elem * elem,   const unsigned int i,    const unsigned int j,
//                           const Point & p);
  //   static Real shape_deriv(const FEType & fe_t,  int extra_order,  const Elem * elem,  const unsigned int i,     const unsigned int j,
//                           const Point & p);
  static double shape_deriv(const unsigned int dim, const FEType & fe_t,   const ElemType t,   const unsigned int i,
                          const unsigned int j,  const double  p[]);
...
// 
  ...
  static double * shape_deriv_function(const unsigned int dim, const FEType & fe_t,  const ElemType t);
...
//    */
  //   static Real shape_second_deriv(const FEType & fe_t,   const Elem * elem,  const unsigned int i,
//                                  const unsigned int j,         const Point & p);
  //   static Real shape_second_deriv (const unsigned int dim,  const FEType & fe_t,     const Elem *elem,
//                                   const unsigned int i,    const unsigned int j,         const Point & p);
  static double shape_second_deriv( const unsigned int dim,    const FEType & fe_t,    const ElemType t,
    const unsigned int i,  const unsigned int j,   const double  p[]);
...
//   shape_second_deriv_function(const FEType & fe_t,       const Elem * elem);
  static double &  shape_second_deriv_function(const unsigned int dim,const FEType & fe_t, const ElemType t);
