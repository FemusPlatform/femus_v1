...
function femus_develop {
  ...
  # main session0
   if [ "$1" == "-help" ]; then
    echo 
    echo "usage"
    echo "femus_develop alias_platform_command" 
    echo "example  femus_develop NumericPlatform"
    else
  if [ "$1" == "" ]; then
    echo 
    echo "usage"
  ...
  dir=$PWD
  # session 1  dbg
  session1=$(qdbus6  $KONSOLE_DBUS_SERVICE $KONSOLE_DBUS_WINDOW newSession)
  command10="cd "$dir
  qdbus6  $KONSOLE_DBUS_SERVICE /Sessions/${session1} runCommand "${command10}"
  command11="$1"
  qdbus6  $KONSOLE_DBUS_SERVICE /Sessions/${session1} runCommand "${command11}"
  command12="femus_application_configure opt"
  qdbus6  $KONSOLE_DBUS_SERVICE /Sessions/${session1} runCommand "${command12}"
  # session e gencase + paraview 
  session2=$(qdbus6  $KONSOLE_DBUS_SERVICE $KONSOLE_DBUS_WINDOW newSession)
  command20="cd "$dir
  qdbus6  $KONSOLE_DBUS_SERVICE /Sessions/${session2} runCommand "${command20}"
  command21="$1"
  qdbus6  $KONSOLE_DBUS_SERVICE /Sessions/${session2} runCommand "${command21}"
  command22="femus_application_configure opt"
  qdbus6  $KONSOLE_DBUS_SERVICE /Sessions/${session2} runCommand "${command22}"
  command23="cd ../../../PLAT_CODES/femus/applications/gencase/"
  qdbus6  $KONSOLE_DBUS_SERVICE /Sessions/${session2} runCommand "${command23}"
  command24="paraview &"
  qdbus6  $KONSOLE_DBUS_SERVICE /Sessions/${session2} runCommand "${command24}"
 fi
 fi
}
 ...
 function femus_workbench {
   if [ "$1" == "-help" ]; then
    echo 
    echo "usage"
    echo "femus_workbench alias_platform_command" 
    echo "example  femus_workbench NumericPlatform"
   else
   #Create sessi data format '<Tab Name/Title>    <Profile Name>   <Working Directory>'
   # main session0
   if [ "$1" == "" ]; then
   ...
   kate -n DATA/Equations_conf.h  DATA/MGFE_conf.h DATA/param_files_msh1.in DATA/SimulationConfiguration.in  DATA/Equations.in   &
   kate -n DATA/GeometrySettings.in DATA/NSproperties.in DATA/RANS_dynamical_properties.in &
   kate -n DATA/Tproperties.in Domain_conf.h    DATA/MaterialProperties.in    DATA/RANS_thermal_properties.in  DATA/Turbulence.in  &
   dir=$PWD
   # session 1  opt
   session1=$(qdbus6  $KONSOLE_DBUS_SERVICE $KONSOLE_DBUS_WINDOW newSession)
   command10="cd "$dir
  ...
  qdbus6  $KONSOLE_DBUS_SERVICE /Sessions/${session1} runCommand "${command13}"
   # session e gencase + paraview 
  session2=$(qdbus6  $KONSOLE_DBUS_SERVICE $KONSOLE_DBUS_WINDOW newSession)
  command20="cd "$dir
  qdbus6  $KONSOLE_DBUS_SERVICE /Sessions/${session2} runCommand "${command20}"
...
function femus_turbulence_compile_lib_dbg {
 if [ "$1" == "-help" ]; then
    echo 
    echo "usage"
    echo "femus_turbulence_compile_lib_dbg   np(num processors)" 
    echo "femus_turbulence_compile_lib_dbg   femus_workbench 1"
   else
...
function intern_femus_show_application_functions {
  ...
  echo "  ${red}femus_gencase_run     <np>${NC}: run gencase using np procs"
  echo "  ${red}femus_gencase_compile <np>${NC}: compile gencase using np procs"
...
function femus_FEMuS_run {
   ...
   if [ "$?" != 0 ]; then
     ...
     mpiexec --oversubscribe -np $pp $FM_MYAPP-$METHOD 2> messages.log
   fi
}
...
if [ "$1" == "-help" ]; then
    echo 
    echo "usage"
    echo "femus_gencase_develop alias_platform_command" 
    echo "example  femus_gencase_develop NumericPlatform"
    else
  #Create session data format '<Tab Name/Title>    <Profile Name>   <Working Directory>'
  # main session0
  if [ "$1" == "" ]; then
    echo 
    echo "usage"
  ...
  cd ../../../PLAT_CODES/femus/applications/gencase/
  femus_application_configure dbg
  dir=$PWD
  command23="cd ../../../PLAT_CODES/femus/applications/gencase/"
  qdbus6  $KONSOLE_DBUS_SERVICE /Sessions/${session2} runCommand "${command23}"
...
# ======================================================================
function femus_gencase_compile {
  ...
  # gencase ------------------------------------------------------
  echo "${red}Now compiling standard gencase application${reset}"

  make clean
  make -j
  mv $FEMUS_DIR/bin/gencase-opt $FEMUS_DIR/bin/gencase;
  rm -rf obj
  cd $ACTUAL_DIR
  export METHOD=$OLD_METHOD
...
if  [ "$1" == "-help" ]; then
    echo 
    echo "usage"
    echo "femus_gencase_run   np (num processors)" 
    echo "example with 2 processors: femus_gencase_run 2 "
   else
   ...
   if [ -d  $FEMUS_DIR/bin ]; then
     ...
     if [ ! -f "$FEMUS_DIR/bin/gencase" ]; then
       ...
       femus_gencase_compile
     fi
     echo run gencase .. $FEMUS_DIR/bin/gencase
     mpiexec --oversubscribe -np $pp $FEMUS_DIR/bin/gencase 
...
#     echo 
#     echo "usage"
#     echo "femus_gencase_run_lib2Dn   np (num processors)" 
#     echo "example with 2 processors: femus_gencase_run_lib2D 2 "
#    else
# if [  "$1" == ""  ]; then pp="1" ;    else    pp=$1;    fi
#    if [ -d  $FEMUS_DIR/bin ]; then
#      echo "found bin directory "
...
#      fi
#      
#      mpiexec --oversubscribe -np $pp gencase_2d 2> messages_gencase.log
#      
#    else
#      echo "${red} ======================================================= "
#      echo " UNABLE TO FIND /USER_APPL/bin DIRECTORY  "
...
#     echo 
#     echo "usage"
#     echo "femus_gencase_run_lib3D   np (num processors)" 
#     echo "example with 2 processors: femus_gencase_run_lib3D 2 "
#    else
# 
# if [  "$1" == ""  ]; then pp="1" ;    else    pp=$1;    fi
#    if [ -d  $FEMUS_DIR/bin ]; then
...
#      fi
#      
#      mpiexec --oversubscribe -np $pp gencase_3d 2> messages_gencase.log
#      
#    else
#      echo "${red} ======================================================= "
#      echo " UNABLE TO FIND /USER_APPL/bin DIRECTORY  "
...
function femus_interpolator_run {
   if [ -d  $FEMUS_DIR/bin ]; then
     echo "found bin directory "
     mpiexec --oversubscribe -np $1 Interpolator 2> messages_interpolator.log
   else
     echo "${red} ======================================================= "
     echo " UNABLE TO FIND /USER_APPL/bin DIRECTORY  "
     echo " CREATE bin DIRECTORY, THEN TYPE "
