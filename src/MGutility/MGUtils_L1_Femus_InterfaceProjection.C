...
// Femus lib control include -------->
#include "Solverlib_conf.h"

// femus lib include ------------>
#include "MGUtils_L1_InterfaceProjection.h"
// #include "MGFE.h"
...
void InterfaceProjection::FillParameters(
  ...
  MEDCoupling::DataArrayIdType* BoundingNodes1 = MEDCoupling::DataArrayIdType::New();
  BoundingNodes1->alloc(_TrgNodes, _SrcCellNodes);
  BoundingNodes1->fillWithValue((mcIdType)-1);
  ...
  MEDCoupling::DataArrayIdType* targetArray = MEDCoupling::DataArrayIdType::New();
  targetArray->alloc(_TrgNodes, 1);
  targetArray->fillWithValue((mcIdType)1);
  ...
  std::vector<mcIdType> TrgConn;                                     // Vector containing the cell node ids
  double* PointsCoords = new double[TrgCellNodes * _SpaceDim];  // Node coord*/inates {xyz xyz xyz}
  ...
  switch (DomainType) {
    case (Boundary):
      ...
      for (int iCell = 0; iCell < _TrgCells; iCell++) {
        ...
        MEDCoupling::MemArray<mcIdType> SrcIntCellsIds =vettore->accessToMemArray();  // Array containing source mesh cell ids
        ...
        int IdSrcCell;
        double NodeCoord[3];  // Vector containing target mesh node coordinates
        ...
        for (int j = 0; j < TrgCellNodes; j++) {
          if (targetArray->getIJ(TrgConn[j], 0) > 0.5) {
            ...
            std::vector<mcIdType> SrcNodesConn;  // connectivity to be filled from getNodeIdsOfCell med function
    ...
    case (Volume):
      ...
      std::vector<mcIdType> SourceConn;
      std::vector<double> ElCoord1;
      //     for(int TNode = 0; TNode < _TrgNodes; TNode++) {
      //
      //
...
void InterfaceProjection::FillParametersProbe(
  ...
  //       elts ->     vector returning ids of found cells.
  MEDCoupling::MCAuto<MEDCoupling::DataArrayIdType> elts;
  MEDCoupling::MCAuto<MEDCoupling::DataArrayIdType> eltsIndex;
  int dimm = coord.size();
  double pos[dimm];
...
void InterfaceProjection::XiEtaCalc_2D(
  ...
  double Co[3 * 3];
  double Coord[9 * 3];
  for (int n_dim = 0; n_dim < _SpaceDim; n_dim++) {
    xm[n_dim] = 0.;
    for (int nnode = 0; nnode < npt_el; nnode++) {
      Coord[nnode + n_dim * npt_el] = _CoordMatrix[n_dim + nnode * _SpaceDim];
      xm[n_dim] += Coord[nnode + n_dim * npt_el] / npt_el;
    }
  }
  // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 (_mgfemap_U->get_FE(1))->normal_g(/*npt_el,*/Coord, xm, normal);   // Lagrange constant(0),linear(1),quad(2)
  // _fe[1]->normal_g(/*npt_el,*/Coord, xm, normal);  // normal in the new med surface
// _fe_D[2]->normal_g(_xxb_qnds_R, _xyz_D, _normal_NS);
...
int InterfaceProjection::IsNodeContained() {
  ...
  if (_GeomType == 1) {
    if (fabs(_XiEtaChi[0]) > 1 + toll || fabs(_XiEtaChi[1]) > 1 + toll ||
        fabs(_XiEtaChi[_SpaceDim - 1]) > 1 + toll)
      Verify = 0;
  }
  if (_GeomType == 0) {
    if (_XiEtaChi[0] < -toll || _XiEtaChi[0] > 1 + toll || _XiEtaChi[1] < -toll || _XiEtaChi[1] > 1 + toll ||
        _XiEtaChi[_SpaceDim - 1] < -toll || _XiEtaChi[_SpaceDim - 1] > 1 + toll)
      Verify = 0;
  }
        ...
        if (BoundingNodes[0] == -1) {
          ...
          for (int iComp = 0; iComp < NComp; iComp++) {
            if (((EqName == "NS0" || EqName == "FSI0" || EqName == "FSIA0" || EqName == "NSA0") &&
                ...
                order == 1) {
              TrgValue = 0.;
              for (int phin = 0; phin < pow(2, _SpaceDim); phin++) {  /// only edge quad and hex
                double val = SourceField->getIJ(BoundingNodes[phin], iComp);
