...
void InterfaceFunctionM::set_mesh_interface_elemID(
  ...
  int n_elements_mg = _mesh_mg->_NoElements[0][Level];  // n MG elements
   int n_nodes_el = _mesh_mg->_GeomEl.n_q[0];  // n MG elements
   int n_nodes_linear_el = _mesh_mg->_GeomEl.n_q[1];  // n MG elements
  int dim_mg = _mesh_mg->_dim;                          // MG dimension
  int n_nod_el = (dim_mg == _space_dim) ? n_nodes_el : n_nodes_linear_el;
  ...
  std::vector<mcIdType> nodes1;  // element nodes
...
void InterfaceFunctionM::set_analytic_field_elem(
  ...
  int n_nod_el =n_nodes_med;//????????????????????????????? MAX_NDOF_FEM;
