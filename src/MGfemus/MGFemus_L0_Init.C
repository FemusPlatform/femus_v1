...
// femus configuration include (from PLAT_USER ->APP/DATA)
...
{  // ============================================================================
  ...
  _size = static_cast<int>(this->comm().size());  assert(_size >= 0);
  _rank = static_cast<unsigned int>(this->comm().rank());  assert(_rank >= 0);
...
P_PROC ( std::cout << " MGFemusInit: PETSC_COMM_WORLD initialized \n"; )
#endif

P_PROC( std::cout << " MGFemusInit: MPI_COMM_WORLD initialized from proc " << _rank << "\n"; )
  // c) Femus initialization ************************************************************
  if (_rank != 0) { std::cout.rdbuf(NULL); } // redirect rank!=0 to null POINTER
