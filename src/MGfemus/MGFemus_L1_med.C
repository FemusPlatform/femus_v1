...
void FEMUS::init_interface(
  ...
  for (unsigned j = 0; j < IDSvec.size(); j++) {
  ...
  int FinerLevel = _mg_utils._dict_geom["nolevels"] - 1;
  ...
  MEDCoupling::DataArrayDouble* MedToMgMapArray = FieldContainingMap->getArray();
...
void FEMUS::getNodeMapAndProcMeshAtLevel(
  ...
  for (unsigned i = 0; i < ProcField->getNumberOfTuples(); i++) {
    if ((int)ProcField->getIJ(i, 0) == proc) CellIdProc.push_back(i);
  }

  mcIdType* CellId = new mcIdType[CellIdProc.size()];  // ARRAY CONTAINING CELL IDS OF SINGLE PROC MESH

  for (unsigned i = 0; i < CellIdProc.size(); i++) CellId[i] = CellIdProc[i];
  ...
  std::vector<mcIdType> FeParCellConn, FeTotCellConn;  // VECTOR FOR PARALLEL AND GLOBAL MESH CONNECTIVITIES
...
void FEMUS::init_interface(
  ...
  std::vector<mcIdType> MgNodesIds, MedNodesIds;
...
MEDCoupling::MEDCouplingFieldDouble* FEMUS::getProcSolution(  // field (out)
  ...
  MyAssert(
      _NodeMap.size() > (unsigned) Level, "FEMUS::getProcSolution _NodeMap not initialized for level " +
