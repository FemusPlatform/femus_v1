...
std::unique_ptr<MGFEbase> MGFEbase::build (const unsigned int dim,  const FEType & fet){
  switch (dim) {
      // 0D
    case 0:   {
        switch (fet.family)  {
          // case CLOUGH:   return std::make_unique<FE<0,CLOUGH>>(fet);
          // case HERMITE:  return std::make_unique<FE<0,HERMITE>>(fet);
          case LAGRANGE: return std::make_unique<MGFE<0,LAGRANGE>>(fet);
          case LAGRANGE_XFEM: return std::make_unique<MGFE<0,LAGRANGE_XFEM>>(fet);
          // case L2_LAGRANGE: return std::make_unique<FE<0,L2_LAGRANGE>>(fet);
          // case HIERARCHIC:  return std::make_unique<FE<0,HIERARCHIC>>(fet);
          // case L2_HIERARCHIC:   return std::make_unique<FE<0,L2_HIERARCHIC>>(fet);
          // case SIDE_HIERARCHIC:  return std::make_unique<FE<0,SIDE_HIERARCHIC>>(fet);
          // case MONOMIAL:   return std::make_unique<FE<0,MONOMIAL>>(fet);
          // case SZABAB:            return std::make_unique<FE<0,SZABAB>>(fet);
          // case BERNSTEIN:    return std::make_unique<FE<0,BERNSTEIN>>(fet);
          // case RATIONAL_BERNSTEIN:    return std::make_unique<FE<0,RATIONAL_BERNSTEIN>>(fet);
          // case XYZ:    return std::make_unique<FEXYZ<0>>(fet);
          // case SCALAR:    return std::make_unique<FEScalar<0>>(fet);
          default:   std::cout<<"ERROR: Bad FEType.family == " << fet.family;
          }
      }
      // 1D
    case 1:  {
        switch (fet.family)    {
          // case CLOUGH:   return std::make_unique<FE<1,CLOUGH>>(fet);
          // case HERMITE:  return std::make_unique<FE<1,HERMITE>>(fet);
          case LAGRANGE:   return std::make_unique<MGFE<1,LAGRANGE>>(fet);
          case LAGRANGE_XFEM:   return std::make_unique<MGFE<1,LAGRANGE_XFEM>>(fet);
          // case L2_LAGRANGE:   return std::make_unique<FE<1,L2_LAGRANGE>>(fet);
          // case HIERARCHIC:    return std::make_unique<FE<1,HIERARCHIC>>(fet);
          // case L2_HIERARCHIC:  return std::make_unique<FE<1,L2_HIERARCHIC>>(fet);
          // case SIDE_HIERARCHIC:  return std::make_unique<FE<1,SIDE_HIERARCHIC>>(fet);
          // case MONOMIAL:     return std::make_unique<FE<1,MONOMIAL>>(fet);
          // case SZABAB:      return std::make_unique<FE<1,SZABAB>>(fet);
          // case BERNSTEIN:   return std::make_unique<FE<1,BERNSTEIN>>(fet);
          // case RATIONAL_BERNSTEIN:  return std::make_unique<FE<1,RATIONAL_BERNSTEIN>>(fet);
          // case XYZ:  return std::make_unique<FEXYZ<1>>(fet);
          // case SCALAR:   return std::make_unique<FEScalar<1>>(fet);
          default:   std::cout<<"ERROR: Bad FEType.family == " << fet.family;
          }
      }
      // 2D
    case 2:  {
        switch (fet.family)   {
          // case CLOUGH:  return std::make_unique<FE<2,CLOUGH>>(fet);
          // case HERMITE:  return std::make_unique<FE<2,HERMITE>>(fet);
          case LAGRANGE:  return std::make_unique<MGFE<2,LAGRANGE>>(fet);
          case LAGRANGE_XFEM:  return std::make_unique<MGFE<2,LAGRANGE>>(fet);
          // case L2_LAGRANGE:  return std::make_unique<FE<2,L2_LAGRANGE>>(fet);
          // case HIERARCHIC: return std::make_unique<FE<2,HIERARCHIC>>(fet);
          // case L2_HIERARCHIC:  return std::make_unique<FE<2,L2_HIERARCHIC>>(fet);
          // case SIDE_HIERARCHIC:  return std::make_unique<FE<2,SIDE_HIERARCHIC>>(fet);
          // case MONOMIAL:  return std::make_unique<FE<2,MONOMIAL>>(fet);
          // case SZABAB:  return std::make_unique<FE<2,SZABAB>>(fet);
          // case BERNSTEIN:  return std::make_unique<FE<2,BERNSTEIN>>(fet);
          // case RATIONAL_BERNSTEIN:  return std::make_unique<FE<2,RATIONAL_BERNSTEIN>>(fet);
          // case XYZ:   return std::make_unique<FEXYZ<2>>(fet);
          // case SCALAR:   return std::make_unique<FEScalar<2>>(fet);
          // case SUBDIVISION:  return std::make_unique<FESubdivision>(fet);
          default: std::cout<<"ERROR: Bad FEType.family == " << fet.family;
          }
      }
      // 3D
    case 3:    {
        switch (fet.family)    {
          // case CLOUGH: std::cout<<"ERROR: Clough-Tocher elements currently only support 1D and 2D";
          // case HERMITE:  return std::make_unique<FE<3,HERMITE>>(fet);
          case LAGRANGE:  return std::make_unique<MGFE<3,LAGRANGE>>(fet);
          // case L2_LAGRANGE:  return std::make_unique<FE<3,L2_LAGRANGE>>(fet);
          // case HIERARCHIC:   return std::make_unique<FE<3,HIERARCHIC>>(fet);
          // case L2_HIERARCHIC:  return std::make_unique<FE<3,L2_HIERARCHIC>>(fet);
          // case SIDE_HIERARCHIC:   return std::make_unique<FE<3,SIDE_HIERARCHIC>>(fet);
          // case MONOMIAL:  return std::make_unique<FE<3,MONOMIAL>>(fet);
          // case SZABAB:  return std::make_unique<FE<3,SZABAB>>(fet);
          // case BERNSTEIN:  return std::make_unique<FE<3,BERNSTEIN>>(fet);
          // case RATIONAL_BERNSTEIN:   return std::make_unique<FE<3,RATIONAL_BERNSTEIN>>(fet);
          // case XYZ:  return std::make_unique<FEXYZ<3>>(fet);
          // case SCALAR:  return std::make_unique<FEScalar<3>>(fet);
          default: std::cout<<"ERROR: Bad FEType.family == " << fet.family;
          }
      }

          default:    std::cout<<"Invalid dimension dim = " << dim;
    }
}
// 
// 
...
double MGFEbase::compute_fe_gauss_points(const int  qp, const double x_elem[] , double InvJac[]){  abort(); return 0.;}

double MGFEbase::compute_fe_face_gauss_points(const int  qp, const double x_elem[]){  abort(); return 0.;}

// template <typename OutputType>
 void MGFEbase::compute_shape_functions (
     const int /* Elem **/ elem, 
     const double qp[]
 ){
//   //-------------------------------------------------------------------------
...
// 
// template <typename OutputType>
// void MGFEbase::print_phi(std::ostream & os) const
//  {
//    for (int iq=0;iq< _n_total_qp;iq++)
//      for (int j=0;j<  _n_shape;j++)
//        os << " phi[" << iq << "][" << j << "]=" << phi[iq* _n_shape+j] << std::endl;
//  }
// 
// template <typename OutputType>
...
// 
// template <typename OutputType>
 // void MGFEbase::print_dphi(
 //     std::ostream & os
 // ) const{
 //  for (int idim=0;idim< _dim;idim++)
 //   for (int iq=0;iq< _n_total_qp;iq++)
 //     for (int j=0;j<  _n_shape;j++)
 //       os << " phi ("<<idim<<")[" << iq << "][" << j << "]=" << dphi[idim*(_n_total_qp*_n_shape)+iq* _n_shape+j] << std::endl;   
 // }
// 
// template <typename OutputType>
...
// 
// template <typename OutputType>
void MGFEbase::determine_calculations()
 {
   this->_calculations_started = true;
...
//  void MGFEbase::print_d2phi(std::ostream & os) const {
//      
//    for (unsigned int i=0; i< _n_total_qp*_n_shape*_dim;i++)
//        for (unsigned int idim=0; idim<_dim;idim++)
//             for (unsigned int jdim=idim; jdim<_dim;jdim++)
//        os << " d2phi[" << idim << "][" << jdim << "]("<<i <<")=" << d2phi[(idim+jdim)*(_n_total_qp*_n_shape)];
//  }
// 
// template <typename OutputType>
