...
// // FE class members
template <>
 MGFE<2,LAGRANGE_XFEM>::MGFE (const FEType & fet) :
   MGFEbase (2,fet),
   ...
   last_edge(INVALID_ELEM)
 {
  ...
  static std::unique_ptr<MGFE<2,LAGRANGE_XFEM>> build (const unsigned int dim, const FEType & type){}
 
template <>
 MGFE<2,LAGRANGE_XFEM>::~MGFE(){}

 
  template <>  const int MGFE<2,LAGRANGE_XFEM>::get_dim()       {return _dim;}  ///< Dimension (3 or 2) used in the 3D FEM (1< _dim < MDIM_FEM)
 template <>  const int MGFE<2,LAGRANGE_XFEM>::get_order()     {return _order;}///< Order of the shape functions 0=cpnst 1=linear 2=quad
 template <>  const int MGFE<2,LAGRANGE_XFEM>::get_NoShape()   {return _NoShape;}  ///< Number of shape functions of the fem
 template <>  const int MGFE<2,LAGRANGE_XFEM>::get_NoElNodes() {return _NoElNodes;}  ///< Number of nodes in the reference element (always ref quad)
 template <>  const int MGFE<2,LAGRANGE_XFEM>::get_GeomType()  {return _GeomType;}   ///< Geom type 1=HEXAHEDRAL 0=TETRAHEDRAL
 template <>  const int MGFE<2,LAGRANGE_XFEM>::get_FamType()   {return _FamType;}    ///< Family  (geom 1: Lagrangian =0  RT =1)
  
   // Gaussian points -----------------------------------------------------------------------------
 template <>  const int MGFE<2,LAGRANGE_XFEM>::get_NoGauss(){ return _NoGauss;}  ///< Number of Gaussian points in 1-2-3D (for example, for HEX27 ngauss[3]=(3,9,27))
  template <> const double  MGFE<2,LAGRANGE_XFEM>::get_xg(int i){          return  _xg[i];}          ///< gaussian points
  template <> const double MGFE<2,LAGRANGE_XFEM>::get_weight(int i){      return _weight[i];}            ///< Weight
  template <> const double & MGFE<2,LAGRANGE_XFEM>::get_phi(){     return  *phi;}           ///< Shape functions
  template <> const double & MGFE<2,LAGRANGE_XFEM>::get_dphidxez_map(){return  *_dphidxez_map;}      ///< Shape derivative functions in gaussian points
  template <> const double & MGFE<2,LAGRANGE_XFEM>::get_dphidxx_map(){ return  *_dphidxx_map;}       ///< Second order shape derivatives in Gaussian points
  
   // Dofs points ----------------------------------------------------------------------------------
 template <>  const double &  MGFE<2,LAGRANGE_XFEM>::get_xnodes(){return   *_xnodes;}              ///< xnode (hex or tetra) in (xi,eta,seta)
 template <>  const int &  MGFE<2,LAGRANGE_XFEM>::get_fem_conn(){ return *_fem_conn;}              ///< dof node coords (respect to _xnodes)
  // template <> const int     MGFE<2,LAGRANGE_XFEM>::get_deg(){return _deg;}                  ///< Polinomial degree
  
   // Nodes (NDOF_FEM) -----------------------------------------------------------------------------
 template <>  const double & MGFE<2,LAGRANGE_XFEM>::get_phi_map_nodes(){return   *_phi_map_nodes;   }          ///< Shape functions
 template <>  const double & MGFE<2,LAGRANGE_XFEM>::get_dphidxez_map_nodes(){return   *_dphidxez_map_nodes;} 
 
// ===========================================================================
 template <> void MGFE<2,LAGRANGE_XFEM>::attach_quadrature_rule (
   QBase *q
 ){ // ===========================================================================
   assert(q);
   this->_qrule = q;
...
// =========================================================================== 
 template <> unsigned int MGFE<2,LAGRANGE_XFEM>::n_shape_functions (
) const {// ===========================================================================
    return MGFE<2,LAGRANGE_XFEM>::n_dofs (this->_elem_type, this->_fe_type.order + this->_p_level);
  }
  
// ===========================================================================
template <> int MGFE<2,LAGRANGE_XFEM>::GetFamilyType(
  int elem_dof, 
  int dim
) {//=========================================================================
...
/// This function writes shape and derivative values at the gaussian points
 template <> void MGFE<2,LAGRANGE_XFEM>::write_c(
...
// ===========================================================================   
 template <> void MGFE<2,LAGRANGE_XFEM>::write(
...
// ===========================================================================
template <> void MGFE<2,LAGRANGE_XFEM>::print_rec_xdmf(
...
// 
template <FEFamily T>
double fe_lagrange_2D_shape_deriv(const ElemType type,
                                const Order order,
                                const unsigned int i,
                                const unsigned int j,
                                const double  p[]);
...
// 
// template <FEFamily T>
// double fe_lagrange_2D_shape_second_deriv(const ElemType type,
//                                        const Order order,
//                                        const unsigned int i,
//                                        const unsigned int j,
//                                        const Point & p);
...
// 
// 
// LIBMESH_DEFAULT_VECTORIZED_FE(2,LAGRANGE_XFEM)
// LIBMESH_DEFAULT_VECTORIZED_FE(2,L2_LAGRANGE)
// 
// =========================================================
template <> double MGFE<2,LAGRANGE_XFEM>::shape(
  const ElemType type,
  const Order order,
  const unsigned int i,
  const double  p[]
){ // ======================================================
  return fe_lagrange_2D_shape<LAGRANGE_XFEM>(type, order, i, p);
}
...
// -------------
// template <> double MGFE<2,LAGRANGE_XFEM>::shape(const Elem * elem,   const Order order,  const unsigned int i,
//                            const Point & p,   const bool add_p_level) {
//   libmesh_assert(elem);
// call the orientation-independent shape functions
//   return fe_lagrange_2D_shape<LAGRANGE_XFEM>(elem->type(), order + add_p_level*elem->p_level(), i, p); }
// -----------
...
// ------------
// template <> double MGFE<2,LAGRANGE_XFEM>::shape(const FEType fet,  const Elem * elem,    const unsigned int i,
//                            const Point & p, const bool add_p_level)
// {  libmesh_assert(elem);  return fe_lagrange_2D_shape<LAGRANGE_XFEM>(elem->type(), fet.order + add_p_level*elem->p_level(), i, p); }
// -----------
...
// =======================================================================
template <> double MGFE<2,LAGRANGE_XFEM>::shape_deriv(
  const ElemType type,  
  const Order order,
  const unsigned int i,  
  const unsigned int j, 
  ...
  return fe_lagrange_2D_shape_deriv<LAGRANGE_XFEM>(type, order, i, j, p); }
// ----------------
...
// -----------------------
// template <>
// double MGFE<2,LAGRANGE_XFEM>::shape_deriv(const Elem * elem,   const Order order,        const unsigned int i,
//                                  const unsigned int j,   const Point & p,    const bool add_p_level)
// {  libmesh_assert(elem);  // call the orientation-independent shape functions
//   return fe_lagrange_2D_shape_deriv<LAGRANGE_XFEM>(elem->type(), order + add_p_level*elem->p_level(), i, j, p);}
// ----------------
...
//  ---------------
// template <> double MGFE<2,LAGRANGE_XFEM>::shape_deriv(const FEType fet,   const Elem * elem,    const unsigned int i,
//                                  const unsigned int j, const Point & p,    const bool add_p_level)
// {   libmesh_assert(elem);   return fe_lagrange_2D_shape_deriv<LAGRANGE_XFEM>(elem->type(), fet.order + add_p_level*elem->p_level(), i, j, p); }
// -----------------
...
// #ifdef LIBMESH_ENABLE_SECOND_DERIVATIVES
// ================================================================
template <> double MGFE<2,LAGRANGE_XFEM>::shape_second_deriv(
  const ElemType type,
  const Order order,
  const unsigned int i,
  const unsigned int j,
  const double  p[]
   ...
   return fe_lagrange_2D_shape_second_deriv<LAGRANGE_XFEM>(type, order, i, j, p); }
...
// ----------------
// template <> double MGFE<2,LAGRANGE_XFEM>::shape_second_deriv(const Elem * elem,  const Order order,
//                                         const unsigned int i,   const unsigned int j,   const Point & p,  const bool add_p_level)
// {   libmesh_assert(elem);   // call the orientation-independent shape functions
//   return fe_lagrange_2D_shape_second_deriv<LAGRANGE_XFEM>(elem->type(), order + add_p_level*elem->p_level(), i, j, p);}
// --------------------
// template <> double MGFE<2,L2_LAGRANGE>::shape_second_deriv(const Elem * elem,
//                                            const Order order,
//                                            const unsigned int i,
//                                            const unsigned int j,
//                                            const Point & p,
...
// 
// template <>
// double MGFE<2,LAGRANGE_XFEM>::shape_second_deriv(const FEType fet,
//                                         const Elem * elem,
//                                         const unsigned int i,
//                                         const unsigned int j,
//                                         const Point & p,
...
// {
//   libmesh_assert(elem);
//   return fe_lagrange_2D_shape_second_deriv<LAGRANGE_XFEM>(elem->type(), fet.order + add_p_level*elem->p_level(), i, j, p);
// }
// 
// 
// 
// template <>
// double MGFE<2,L2_LAGRANGE>::shape_second_deriv(const FEType fet,
//                                            const Elem * elem,
//                                            const unsigned int i,
//                                            const unsigned int j,
//                                            const Point & p,
...
template <> double   MGFE<2,LAGRANGE_XFEM>:: compute_fe_gauss_points(
...
template <> double   MGFE<2,LAGRANGE_XFEM>:: compute_fe_face_gauss_points(
   const int  gp, // gaussian point gp
   const double xyz[] // x_elem[xyz(1),xyz(2), xyz(3),   xyz(_Nshape)] 
 ){
...
// ================================================================================================
template <> 
double MGFE<2,LAGRANGE_XFEM>::ComputeInverseMatrix_vol(double Matrix[], double InvMatrix[]
...
//===============================================================================================
template <> 
double MGFE<2,LAGRANGE_XFEM>::ComputeInverseMatrix_sur(double Matrix[], double InvMatrix[]) {
  double det = (Matrix[0]);
  double idet = 1. / det;
...
//===============================================================================================
template <> 
double MGFE<2,LAGRANGE_XFEM>::ComputeInverseMatrix(double Matrix[], double InvMatrix[], int dim) {
  double det = 0.;
...
//===============================================================================================
template <> 
double MGFE<2,LAGRANGE_XFEM>::Tj_interp_lin(double *u,int j, double *xi,int ndof,int idim) {
...
//===============================================================================================
template <> 
double MGFE<2,LAGRANGE_XFEM>::dTjdxi_lin(double *u,int j, double *xi,int i,int ndof,int idim) {
  double sum=0.; /*int ndof=8;*/
...
template <>
void MGFE<2,LAGRANGE_XFEM>::normal_g(
...
//=============================================================
template <> void  MGFE<2,LAGRANGE_XFEM>::Oxy_face(
...
// =================================================================================
template <> void MGFE<2,LAGRANGE_XFEM>::set_par(
  std::string fileh5,
  int dim_in,
  int order_in,
  int shapes_quad,
 ...
 double *point=new double[_dim];
  ...
  get_refspace_nodes(geom_elem_name,xnodes);// nodes[(x,y)_0,(x,y)_1.... ]
    ...
    for(int j = 0; j < _NoShape; j++) {  // shapse
      ...
      _phi_map_nodes[j*_NoElNodes+i]=fe_lagrange_2D_shape<LAGRANGE_XFEM>(elem_name , elem_order, j, point);//Rec_Quad_Phi(j, point,_dim);
      // _dphidxez_map_nodes[dphi1(xp1)/de1,dphi1(xp2)/de1,....dphi2(xp1)/de1,dphi2(xp2)/e1,...  ]
    for(int dir = 0; dir < _dim; dir++) {  //  derivatives ++++++
        _dphidxez_map_nodes[(j + dir *_NoShape)*_NoElNodes+i] =fe_lagrange_2D_shape_deriv<LAGRANGE_XFEM>(elem_name, elem_order, j,dir, point);// Rec_Quad_DPhi(j,point,_dim,dir);     // dphi/dxi
      }
    }
  }
   delete[]xnodes;
// gaussian points --------------------------------------------------------->
 for(int i = 0; i < _NoGauss; i++) {
    ...
    for(int j = 0; j < _NoShape; j++) {     // loop over test function id
      phi[j*_NoGauss+i] =fe_lagrange_2D_shape<LAGRANGE_XFEM>(elem_name , elem_order, j, point);
      for(int dir = 0; dir < _dim; dir++) {  //  derivatives ++++++
        _dphidxez_map[(j + dir * _NoShape) * _NoGauss + i] =fe_lagrange_2D_shape_deriv<LAGRANGE_XFEM>(elem_name, elem_order, j,dir, point);
        for(int dir2 = 0; dir2 < _dim; dir2++)// dir +dir2=0->d^2phi/dxi^2; dir +dir2=2->d^2phi/deta^2; dir +dir2=1->d^2phi/deta dxi;
          ...
          fe_lagrange_2D_shape_second_deriv<LAGRANGE_XFEM>(elem_name, elem_order,j,dir +dir2, point);
      }// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    }
  } 
  delete []point;
...
template <> double MGFE<2,LAGRANGE_XFEM>::Jac(    //  jacobean ->
...
/// This function computes the  derivatives at
/// the nodal point:
template <> double MGFE<2,LAGRANGE_XFEM>::Jac_nodes( 
...
// ==========================================
template <> double MGFE<2,LAGRANGE_XFEM>::JacSur(
...
template <> double MGFE<2,LAGRANGE_XFEM>::JacobianOnGauss(  // 3D  jacobean ->
...
// ========================================================================
template <> double MGFE<2,LAGRANGE_XFEM>::JacOnGivenCanCoords(
...
// ================================================================
template <> double MGFE<2,LAGRANGE_XFEM>::JacobianOnPoint(
...
//==============================================================================================
template <> void MGFE<2,LAGRANGE_XFEM>::get_dphi_on_given_node(const int dim, double ElemCoords[], double CanPos[], double dphi[]) {
...
template <> void MGFE<2,LAGRANGE_XFEM>::get_phi_gl_g(
...
template <> void MGFE<2,LAGRANGE_XFEM>::get_dphi_gl_g(
const int qp,           // gaussian point <
...
template <> void MGFE<2,LAGRANGE_XFEM>::get_ddphi_gl_g(
const int qp,           // gaussian point <
const double InvJac[],  // Jacobean
...
template <> void MGFE<2,LAGRANGE_XFEM>::get_dphi_node(
...
template <>  bool MGFE<2,LAGRANGE_XFEM>::phys_to_ref(double xref[],double xphys[],const double Xel[], int max_it, double tol) {
    ...
    for (int it = 0; it < max_it; ++it) {
        ...
        for (int i = 0; i < _NoShape; ++i) {
            // const double Ni = Rec_Quad_Phi(i, p, /*_dim=*/2);
             const double Ni = fe_lagrange_2D_shape<LAGRANGE_XFEM>(QUAD9,SECOND,i, p);
            // const double dN_dxi  = Rec_Quad_DPhi(i, p, /*_dim=*/2, /*dir=*/0);
              const double dN_dxi  = fe_lagrange_2D_shape_deriv<LAGRANGE_XFEM>(QUAD9,SECOND, i,0, p);
            // const double dN_deta = Rec_Quad_DPhi(i, p, /*_dim=*/2, /*dir=*/1);
            const double dN_deta = fe_lagrange_2D_shape_deriv<LAGRANGE_XFEM>(QUAD9,SECOND, i,1, p);
        ...
        for (int ls = 0; ls < 10; ++ls) {
            ...
            for (int i = 0; i < _NoShape; ++i) {
              ...
              const double Ni = fe_lagrange_2D_shape<LAGRANGE_XFEM>(QUAD9,SECOND,i, p2);;
