  ...
  for (int ivar = 0; ivar < nvars; ivar++) {
    ...
    for (int j = 0; j < n_shape; j++) {
      for (int jdim = 0; jdim < _ndim_B; jdim++)
        u_int[ivar * dim + jdim] += phi[j + jdim * n_shape] * uold[j + (ivar + ivar0) * _el_dof_D[0]];
    }
  }
  return;
}
...
void MGSolDA::interp_el_sol(
  ...
  for (int ivar = ivar0; ivar < ivar0 + nvars; ivar++) {
    uold[ivar] = 0.;
    for (int eln = 0; eln < n_shape; eln++) { uold[ivar] += phi[eln] * uold_b[eln + ivar * _el_dof_D[0]]; }
  }
  return;
}
...
void MGSolDA::interp_el_gdx(
  ...
  for (int ivar = 0; ivar < nvars; ivar++) {
    for (int jdim = 0; jdim < _ndim_B; jdim++) uold_dx[ivar * _ndim_B + jdim] = 0.;  // set zero
    // interpolation with shape functions
    for (int j = 0; j < n_shape; j++) {
      for (int jdim = 0; jdim < _ndim_B; jdim++)
        uold_dx[ivar * _ndim_B + jdim] += dphi[j + jdim * n_shape] * uold_b[j + (ivar + ivar0) * _el_dof_D[0]];
    }
  }
  return;
}
...
void MGSolDA::interp_el_gddx(
  ...
  // variable loop
  for (int ivar = 0; ivar < nvars; ivar++) {
    for (int jdim = 0; jdim < _ndim_B * _ndim_B; jdim++)
      uold_dx[ivar * _ndim_B * _ndim_B + jdim] = 0.;  // set zero
    // interpolation with shape functions
    for (int eln = 0; eln < n_shape; eln++) {
      for (int jdim = 0; jdim < _ndim_B * _ndim_B; jdim++)
        uold_dx[ivar * _ndim_B * _ndim_B + jdim] +=
            dphi[eln * _ndim_B * _ndim_B + jdim] * uold_b[eln + (ivar + ivar0) * _el_dof_D[0]];
    }
  }
  return;
}
...
//  SET FUNCTIONS
// ========================================================================
// void MGSolDA::set_x_aux(int i, int k, double val) {
//   x_aux[i]->set(k, val);
//   return;
// }  // set the field
// void MGSolDA::set_d_aux(int i, int k, double val) {
//   d_aux[i]->set(k, val);
//   return;
// }
// void MGSolDA::set_sol(int i, int k, double val) {
...
// //  GET FUNCTIONS
// // ========================================================================
// // double MGSolDA::get_x_aux(int i, int k) { return ((*x_aux[i])(k)); }
// // double MGSolDA::get_d_aux(int i, int k) { return ((*d_aux[i])(k)); }
// // double MGSolDA::get_sol(int i, int k) { return ((*x_old[i][_NoLevels - 1])(k)); }
// 
// // ==========================================================================================
...
//     
//     ) const {             // ============================================================== 
// //       double xx[_el_dof_D[0]*2];   int iix[_el_dof_D[0]*2];
...
//       uold[id + (ivar + kvar0) * _el_dof_D[0]] = ((*x_old[i_step][_NoLevels-1])(kdof_top));  // element sol
// //      iix[id+ivar *_el_dof_D[0]]= kdof_top ;
// //    xx[id+ivar *_el_dof_D[0]]= uold[id + (ivar + kvar0) * _el_dof_D[0]];
//     }  // end quadratic ------------------------------------------------
//   }
//   return;
// }
...
//       uold[id + (ivar + kvar0) * _el_dof_D[0]] = ((*x_old[0][_NoLevels - 1])(kdof_top));  // element sol
//     }  // end quadratic ------------------------------------------------
//   }
//   return;
// }
...
//       uold[id + (kvar0 + ivar) * _el_dof_D[0]] = val;  // element sol
//     }  // end quadratic ------------------------------------------------
//   }
//   return;
// }
...
//       const int kdof_top =
//           _node_dof[_NoLevels - 1][el_conn[id] + (ivar + ivar0) * offset];               // dof from top level
//       uold[id + (kvar0 + ivar) * _el_dof_D[0]] = ((d_aux[istep][_NoLevels - 1])(kdof_top));  // element sol
//     }  // end quadratic ------------------------------------------------
...
// // ==========================================================================
// void MGSolDA::set_cp_vector(int nvars[],const int& vec_from, const int& vec_to) {
...
//       if (ivar >= nvars[2])   el_nds = NDOF_P;  // quad and linear
...
//              iel++) {
//           int elem_gidx = (iel + _mgmesh._off_el[0][iproc * _NoLevels + Level]) * _el_dof_D[0];
// 
...
// ====================================================================
void MGSolDA::get_el_dof_indices(
  ...
  for (int id = 0; id < _el_dof_D[0]; id++) {
    // quadratic -------------------------------------------------
    if (id < el_dof[2])
      for (int ivar = 0; ivar < nvars[2]; ivar++) {  // ivarq is like idim
    ...
    //     // linear -----------------------------
    if (id < el_dof[1])
      for (int ivar = 0; ivar < nvars[1]; ivar++) {  // ivarq is like idim
        const int indx_loc_ql = id + ivar * el_dof[1] + nvars[2] * el_dof[2];
        const int indx_glob = el_conn[id] + (ivar + nvars[2]) * offset;
    ...
    //     // piecewise -----------------------------
    if (id < el_dof[0])
      for (int ivar = 0; ivar < nvars[0]; ivar++) {  // ivarq is like idim
        const int indx_loc_ql = id + ivar * el_dof[0] + nvars[2] * el_dof[2] + nvars[1] * el_dof[1];
        const int indx_glob = id + iel * el_dof[0] + (ivar + nvars[2] + nvars[1]) * offset;
...
#ifdef TWO_PHASE_LIB
void MGSolDA::set_mgcc(MGSolCC& cc) { _msolcc = &cc; }
#endif
