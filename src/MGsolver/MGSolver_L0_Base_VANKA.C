...
double MGSolBase0::Vanka_test_B(
  int Level  ///<  level
) {// =============================================================================================
  TRACKING_FUN(printf(" --> MGSolverBase0_VANKA: MGSolBase0VANKA::Vanka_test \n  ");) 
// ================================================================================================    
...
double MGSolBase0::MGStep_Vanka_B(
    int Level,            // Level
  ...
  TRACKING_FUN(printf(" --> MGSolverBase0_VANKA: MGSolBase0::MGStep_Vanka \n  ");) 
  // ==============================================================================================
  std::pair<int, double> rest(0, 0.);
  if (Level == 0) {  // coarse level --------------------------------------------------------------
    Vanka_solve(Level, *A[Level], *x[Level], *b[Level], Eps1, MaxIter, true);   // coarse solution
    res[Level]->resid(*b[Level], *x[Level], *A[Level]);// coarse residual
P_CONV(std::cout << " Coarse res " << MaxIter << " " << res[Level]->l2_norm() << std::endl;)
  }       // --------------------------------------------------------------------------------------
  else {  // fine levels
...
P_CONV(std::cout << " Pre Lev " << Level << " res " << res[Level]->l2_norm();) //  CC +++++++++++++++
TIME(std::clock_t end_time = std::clock();std::cout << " time =" << double(end_time - start_time) / CLOCKS_PER_SEC << std::endl;)
    ...
    x[Level - 1]->close();    x[Level - 1]->zero();
...
TIME(start_time = std::clock();)    //  TC +++++++++++++++
    //  postsmooting
    int Nc_post1 = Nc_post;
    if (Level < _NoLevels - 1) { Nc_post1 *= 2; }
    Vanka_solve(Level, *A[Level], *x[Level], *b[Level], Eps1, Nc_post1, direct);
    res[Level]->resid(*b[Level], *x[Level], *A[Level]);
P_CONV(std::cout << " Post Lev " << Level << " res " << res[Level]->l2_norm();)
TIM(end_time = std::clock();std::cout << " time =" << double(end_time - start_time)/CLOCKS_PER_SEC << std::endl;)
        // ----------------  end postsmoothing ---------------------------
  }
  // end cycle -------------------------------------
  res[Level]->close();
...
void MGSolBase0::Vanka_solve_B(
  ...
  DenseMatrixM Mat;  DenseVectorM sol, loc_rhs;

  // Petsc variables init
  PetscErrorCode ierr;  PetscInt ncols;
  ...
  int el_conn[27];                       // element connectivity
  double xx_qnds[3 * 27];

  vector<double> x_loc[offset];  solution->localize(*x_loc);
  vector<double> b_loc[offset];  rhs->localize(*b_loc);
  ...
  for (int ismooth = 0; ismooth <= nsm; ismooth++) {             // smooth loop
    for (int iel = 0; iel < (nel_e - nel_b); iel++) {            // loop over subdomains
      ...
      if (isdirect == true) {
        ...
        Mat.resize(size, size);        sol.resize(size);    loc_rhs.resize(size);
        ...
        for (int inode = 0; inode < size; inode++) {  // loop over ndof
          ...
          for (int i = 0; i < size; i++)  loc_rhs(inode) += matr[i + size * inode] * (*x_loc)[idx[i]];  // sum diagonal values
          for (int i = 0; i < ncols; i++) loc_rhs(inode) -= vals[i] * (*x_loc)[cols[i]];  // subtract all row values
          ierr = MatRestoreRow(matrix->mat(), idx[inode], &ncols, &cols, &vals);
        }  // end inode

        Mat.zero();
        for (int j = 0; j < size; j++)  for (int i = 0; i < size; i++) { Mat(j, i) = matr[i + j * size]; }
        Mat.lu_solve(loc_rhs, sol);  // Solve Mat*sol=loc_rhs with libmesh lu decomposition
        for (int i = 0; i < size; i++) { (*x_loc)[idx[i]] = sol(i); }
        // We save the solution only after the nsm-th smoothing cycle
        if (ismooth == nsm) for (int i = 0; i < size; i++) { (*x[Level]).set(idx[i], sol(i)); }
      }
      if (isdirect == false) {
        ...
        // We save the solution only after the nsm-th smoothing cycle
        if (ismooth == nsm) for (int i = 0; i < size; i++) { (*x[Level]).set(idx[i], (*x_loc)[idx[i]]); }
      }
    }  // end subdom loop
  }    // end ismooth loop
  return;
