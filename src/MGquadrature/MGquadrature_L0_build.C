...
// ==========================================================================
//---------------------------------------------------------------
std::unique_ptr<QBase> QBase::build (
...
//       }
    ...
    case QTRAP:  {
// #ifdef DEBUG
//         if (_order > FIRST) {  std::cout << "WARNING: Trapezoidal rule provides only" << std::endl
//                          << " FIRST order!" << std::endl;  }
// #endif
        return std::make_unique<QTrap>(_dim);
      }
    // case QGRID:  return std::make_unique<QGrid>(_dim, _order);
    // case QGRUNDMANN_MOLLER:   return std::make_unique<QGrundmann_Moller>(_dim, _order);
    // case QMONOMIAL:   return std::make_unique<QMonomial>(_dim, _order);
    // case QGAUSS_LOBATTO:  return std::make_unique<QGaussLobatto>(_dim, _order);
    // case QCONICAL:  return std::make_unique<QConical>(_dim, _order);
    // case QNODAL:  return std::make_unique<QNodal>(_dim, _order);
    default: std::cout<<"ERROR: Bad qt=" << _qt;abort();
    }
}
