...
void QConical::init_1D(const ElemType, unsigned int){
  ...
  double *pt1d=new double[1];
   gauss1D.get_points(pt1d); _points[0]=pt1d[0];
   
  // _weights.swap(gauss1D.get_weights());
  gauss1D.get_weights(pt1d);_weights[0]=pt1d[0];
  delete []pt1d;
...
void QConical::conical_product_tri()
{
  ...
  std::pair<double, double> old_range(-1.0L, 1.0L);
  std::pair<double, double> new_range( 0.0L, 1.0L);
  gauss1D.scale(old_range, new_range);
  ...
  gauss1D.qp(pt1d,0);     assert (pt1d[0]> 0.0);
  gauss1D.qp(pt1d,np-1);  assert (pt1d[0]<= 1.0);
  jac1D.qp(pt1d,0);       assert (pt1d[0]>= 0.0);//assert (jac1D.qp(0)(0)>= 0.0);
  jac1D.qp(pt1d,np-1);    assert (pt1d[0]<= 1.0);// assert (jac1D.qp(np-1)(0)<= 1.0);
  ...
  for (unsigned int i=0; i<np; i++)
    for (unsigned int j=0; j<np; j++) {
      ...
      jac1D.qp(pt1d,j); _points[gp*_dim+0]=pt1d[0];//s[j];
      jac1D.qp(pt1d,i); _points[gp*_dim+1]=pt1d[0] * _points[gp*_dim+0]; //r[i]*(1.-s[j])
...
double *pt1d=new double[1];
  ...
  gauss1D.qp(pt1d,0);     assert (pt1d[0]> 0.0);
  gauss1D.qp(pt1d,np-1);  assert (pt1d[0]<= 1.0);
  jacA1D.qp(pt1d,0);       assert (pt1d[0]>= 0.0);//assert (jac1D.qp(0)(0)>= 0.0);
  jacA1D.qp(pt1d,np-1);    assert (pt1d[0]<= 1.0);// assert (jac1D.qp(np-1)(0)<= 1.0);
  jacB1D.qp(pt1d,0);       assert (pt1d[0]>= 0.0);//assert (jac1D.qp(0)(0)>= 0.0);
  jacB1D.qp(pt1d,np-1);    assert (pt1d[0]<= 1.0);// assert (jac1D.qp(np-1)(0)<= 1.0);
  // Resize the points and weights vectors
  ...
  for (unsigned int i=0; i<np; i++)
    for (unsigned int j=0; j<np; j++)
      for (unsigned int k=0; k<np; k++)    {
          ...
          jacB1D.qp(pt1d,k); _points[gp*_dim+0]=pt1d[0];    //t[k];
          jacA1D.qp(pt1d,j); _points[gp*_dim+1]=pt1d[0] * _points[gp*_dim+0]; //s[j]*(1.-t[k]);
          gauss1D.qp(pt1d,i);_points[gp*_dim+2]= pt1d[0]* _points[gp*_dim+0]* _points[gp*_dim+1]; //r[i]*(1.-s[j])*(1.-t[k]);
  ...
  for (unsigned int i=0; i<np; ++i)
    for (unsigned int j=0; j<np; ++j)
      for (unsigned int k=0; k<np; ++k, ++q) {
        gauss1D.qp(pt1d,i);  const double xi=pt1d[0];
        gauss1D.qp(pt1d,j);  const double yj=pt1d[0];
        jac1D.qp(pt1d,k);    const double zk=pt1d[0];

          _points[q*_dim+0] = (1.-zk) * xi;
          _points[q*_dim+1] = (1.-zk) * yj;
          _points[q*_dim+2] = zk;
