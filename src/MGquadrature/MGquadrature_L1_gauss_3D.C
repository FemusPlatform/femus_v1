...
// #if LIBMESH_DIM == 3
  ...
  switch (_type)
    ...
    case HEX27:
      ...
      //---------------------------------------------
    case TET4:    case TET10:    case TET14:      {// Tetrahedral quadrature rules
        switch(get_order())   {
            // Taken from pg. 222 of "The finite element method," vol. 1
            // ed. 5 by Zienkiewicz & Taylor
          case CONSTANT:          case FIRST:    { // Exact for linears
            _n_points=1;  _points=new double[3];  _weights=new double[1];
              _points[0] = .25;              _points[1] = .25;              _points[2] = .25;
          ...
          case SECOND:  {
              ...
              const double a = .585410196624969;              const double b = .138196601125011;
              _points[0] = a;              _points[0*_dim+1] = b;              _points[0*_dim+2] = b;
              _points[1] = b;              _points[1*_dim+1] = a;              _points[1*_dim+2] = b;
              _points[2] = b;              _points[2*_dim+1] = b;              _points[2*_dim+2] = a;
              _points[3] = b;              _points[3*_dim+1] = b;              _points[3*_dim+2] = b;
              _weights[0] = double(1.)/24.;
              _weights[1] = _weights[0];              _weights[2] = _weights[0];   _weights[3] = _weights[0];
              return;
            }
          ...
          case THIRD: {
              if (allow_rules_with_negative_weights){
                  _n_points=5;  _points=new double[5*_dim]; _weights=new double[5];
                  _points[0*_dim+0] = .25;          _points[0*_dim+1] = .25;           _points[0*_dim+2] = .25;
                  _points[1*_dim+0]= .5;            _points[1*_dim+1]= double(1.)/6.;  _points[1*_dim+2]= double(1.)/6.;
                  _points[2*_dim+0]= double(1.)/6.; _points[2*_dim+1]= .5;             _points[2*_dim+2]= double(1.)/6.;
                  _points[3*_dim+0]= double(1)/6;   _points[3*_dim+1]= double(1)/6;    _points[3*_dim+2]= .5;
                  _points[4*_dim+0]= double(1)/6;   _points[4*_dim+1]= double(1)/6;    _points[4*_dim+2]= double(1)/6;

                  _weights[0] = double(-2.)/15.;    _weights[1] = .075;
                  _weights[2] = _weights[1];        _weights[3] = _weights[1];   _weights[4] = _weights[1];

                  return;
                } // end if (allow_rules_with_negative_weights)
              else                {
                std::cout << "Notimplemented"; abort();
             ...
             _n_points=14;  _points=new double[ _n_points*_dim]; _weights=new double[ _n_points];
              ...
              for (unsigned int i=0; i<2; ++i)         {
                  ...
                  // for(int kdim=0;kdim<_dim;kdim++){
                   _points[(offset + 0)*_dim+0] = a[i];  _points[(offset + 0)*_dim+1] = a[i];   _points[(offset + 0)*_dim+2] =  a[i];
                   _points[(offset + 1)*_dim+0] = a[i];  _points[(offset + 1)*_dim+1] =  b;     _points[(offset + 1)*_dim+2] =  a[i];
                   _points[(offset + 2)*_dim+0] = b;     _points[(offset + 2)*_dim+1] =  a[i];  _points[(offset + 2)*_dim+2] =  a[i];        
                   _points[(offset + 3)*_dim+0] = a[i];  _points[(offset + 3)*_dim+1] =  a[i];  _points[(offset + 3)*_dim+2] =  b;
...

              {
                ...
                // Here are the permutations.  Order of these is not important,
                // all have the same weight
                _points[(offset + 0)*_dim+0] = b   ;  _points[(offset + 0)*_dim+1] =    b;   _points[(offset + 0)*_dim+2] = a[2];
                _points[(offset + 1)*_dim+0] = b   ;  _points[(offset + 1)*_dim+1] = a[2];   _points[(offset + 1)*_dim+2] = a[2];
                _points[(offset + 2)*_dim+0] = a[2];  _points[(offset + 2)*_dim+1] = a[2];   _points[(offset + 2)*_dim+2] =    b;
                _points[(offset + 3)*_dim+0] = a[2];  _points[(offset + 3)*_dim+1] =    b;   _points[(offset + 3)*_dim+2] = a[2];
                _points[(offset + 4)*_dim+0] =    b;  _points[(offset + 4)*_dim+1] = a[2];   _points[(offset + 4)*_dim+2] =    b;    
                _points[(offset + 5)*_dim+0] = a[2];  _points[(offset + 5)*_dim+1] =    b;   _points[(offset + 5)*_dim+2] =    b;
              ...
              //       _points[0](0) = 0.25;        _points[0](1) = 0.25;
              //       _points[0](2) = 0.25;

              //       {
              // const double a = 0.;
              // const double b = double(1)/3;

              // _points[1](0) = a;              // _points[1](1) = b;         // _points[1](2) = b;

              // _points[2](0) = b; _points[2](1) = a; _points[2](2) = b;

              // _points[3](0) = b; _points[3](1) = b; _points[3](2) = a;

              // _points[4](0) = b; _points[4](1) = b; _points[4](2) = b;
              //       }
              //       {
              // const double a = double(8)/11; const double b = double(1)/11;

              // _points[5](0) = a;    _points[5](1) = b; _points[5](2) = b;

              // _points[6](0) = b;  _points[6](1) = a; _points[6](2) = b;

              // _points[7](0) = b; _points[7](1) = b; _points[7](2) = a;
              ...
              //       }
              //       {
              // const double a = 0.066550153573664;
              // const double b = 0.433449846426336;
              ...
              //       _weights[14] = _weights[9];
                for (unsigned int j=0; j<14; ++j) std::cout<<  _weights[ j] << "  (" <<  _points[j*_dim+0] << "  "  <<    _points[j*_dim+1] << "  "<<    _points[j*_dim+2] << " ) \n ";
          ...
          case SIXTH:  {
             _n_points=24;  _points=new double[ _n_points*_dim]; _weights=new double[ _n_points];
              // _points.resize (24);              _weights.resize(24);

              // The raw data for the quadrature rule.
              const double rule_data[4][4] = {
            ...
            // below.
          case SEVENTH:   case EIGHTH:   {
              if (allow_rules_with_negative_weights)   {
                 _n_points=45;  _points=new double[ _n_points*_dim]; _weights=new double[ _n_points];
                  // _points.resize (45);                  _weights.resize(45);

                  // The raw data for the quadrature rule.
                  const double rule_data[7][4] = {
          ...
          default: {
              ...
              //     return;
              //   }
            std::cout<< " Notimplemented bu it is possible"; abort();
            }
          }
      } // end case TET
      ...
      //---------------------------------------------
    case PRISM6:    case PRISM15:    case PRISM18:    case PRISM20:    case PRISM21:   {    // Prism quadrature rules
        // We compute the 3D quadrature rule as a tensor
        // product of the 1D quadrature rule and a 2D
        // triangle quadrature rule
        QGauss q1D(1,_order);   QGauss q2D(2,_order);
        // Initialize
        q1D.init(EDGE2, _p_level);  q2D.init(TRI3, _p_level);
    ...
    case PYRAMID5:   case PYRAMID13:  case PYRAMID14:  case PYRAMID18:   {
      ...
      //---------------------------------------------
      // Unsupported type
    default:  std::cout<< "ERROR: Unsupported type: " << _type;return;
    }
// #endif
}
