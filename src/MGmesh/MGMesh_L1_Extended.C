...
MeshExtended::MeshExtended(const ParallelObjectM& comm, MGUtils& mgutils, MGGeomElBase& geomel)
...
//   if (_mgutils._FieldClass->_EquationsToAdd["MG_DynamicalTurbulence"] != 0) {
     if (_mgutils._TurbParameters != NULL) {
...
void MeshExtended::read_bc_id(int Level) {
 ...
 const int el_lin_nnodes_bd= _GeomEl.n_l[1];
  ...
  for (int iproc = 0; iproc < _n_subdom; iproc++) {
    ...
    for (int iel = nel_b; iel < nel_e; iel++) {
      for (int inode = 0; inode < el_lin_nnodes_bd; inode++) {
        // get the global node number
        int el_conn = _el_map[0][iel * el_nnodes + inode];
...
void MeshExtended::read_mat_id(int /*Level*/) {
  ...
  // Getting dataset
  hid_t dtset = H5Dopen(file_id,("/ELEMS"+std::to_string(_GeomEl.n_q[0])+"/MAT/MAT").c_str(),H5P_DEFAULT);
  hid_t filespace = H5Dget_space(dtset); /* Get filespace handle first. */
  hsize_t dims[2];
  hid_t status = H5Sget_simple_extent_dims(filespace, dims, NULL);
  if (status < 0) {
...
void MeshExtended::B_dist(const int Level) {
 ...
 const int el_nnodes_bd = _GeomEl.n_q[1];  // element nodes
 const int el_lin_nnodes_bd= _GeomEl.n_l[1];
  double xm[3]/*, xmb[3]*/;    // volume element center coordinates
  double xx_qnds[3 * 27];    // volume element coordinates
//   double xxb_qnds[3 * 9];  // boundary element coordinates
  int el_conn[27];                   // volume element connectivity
//   int el_connb[9];                 // boundary element connectivity
  ...
  std::string GroupString = _mgutils._dict_config["WallGroups"];
  std::string temps;
  ...
  int icount=0; 
  std::vector<int>bdnodes(_NoNodes[_NoLevels-1]);
  for(int inode=0;inode<_NoNodes[_NoLevels-1];inode++){
       for (int i = 0; i < NumberOfWalls; i++) {
          if (fabs(_bc_id[inode]) == WallGroupsIDs[i]) {
            bdnodes[icount]=inode; icount++;
           break;
          }
    }
  }
  ...
  for (int iel = 0; iel < nel_e - nel_b; iel++) {  // i proc elements are nel_e - nel_b
    ...
    for (int idim = 0; idim < _dim; idim++) {
      xm[idim] = 0.;
      for (int l1 = 0; l1 < el_nnodes; l1++) { xm[idim] += xx_qnds[l1 + idim * el_nnodes]; }
      xm[idim] /= el_nnodes;
    }
    ...
    double dminq = 1.e+20;
      ...
      for(int ibnode=0;ibnode<icount;ibnode++){
            double dq = 0.;
          // element center and distance from walldq
          for (int idimb = 0; idimb < _dim; idimb++) {
            dq += (xm[idimb] - _xyz[bdnodes[ibnode]+idimb*_NoNodes[_NoLevels-1]])*(xm[idimb] - _xyz[bdnodes[ibnode]+idimb*_NoNodes[_NoLevels-1]]);
          }
          if (dq < dminq) { dminq = dq; }
        }  // END OF WALL DISTANCE CALCULATION ---------------------------------------------------------
     _dist[iel + nel_b] = sqrt(dminq);  // cyl and annul    
  }
...
// 
//         // determining whether group el_connb[el_nnodes_bd-1] is a wall group or not
//         bool IsAWallGr = false;
//         for (int i = 0; i < NumberOfWalls; i++) {
//           if (fabs(_bc_id[el_connb[el_nnodes_bd - 1]]) == WallGroupsIDs[i]) {
//             IsAWallGr = true;
//             break;
//           }
//         }
//         if (IsAWallGr) {  // CALCULATION PERFORMED ONLY IF  EL_CONNB[el_nnodes_bd-1] IS A WALL GROUP -------
//           double dq = 0.;
//           // element center and distance from walldq
//           for (int idimb = 0; idimb < _dim; idimb++) {
//             xmb[idimb] = 0.;
//             for (int l1b = 0; l1b < el_nnodes_bd; l1b++) { xmb[idimb] += xxb_qnds[l1b + idimb * el_nnodes_bd]; }
//             xmb[idimb] /= el_nnodes_bd;
...
if(_dim==2){// ----------------------- 2d -------------------->
  ...
  for(int i=0;i<el_nnodes;i++)  nodesinv2[i]= nodesinv[i];                       
  for(int i=0;i<3;i++)  nodesinvbd2[i]= nodesinvbd[i]; 
  }
   if(el_nnodes==6){
...
// #endif
  ...
  int el_conn[27];
  //   int       elb_conn[9];
  double* xx_qnds = new double[3 * 27];
  ...
  const int pt_sides_bd = el_nnodes_bd;
  //
  //   // init  -------------------------------------------
...
//   int ielemcount = 0;
  ...
  //
  mcIdType* conn_bd;
  conn_bd = new mcIdType[n_elements * el_sides * pt_sides_bd];
  mcIdType* conn;
  conn = new mcIdType[n_elements * nodes_el];
  ...
  for (int iproc = 0; iproc < _n_subdom; iproc++) {
    ...
    for (int el = 0; el < (nel_e - nel_b); el++) {
      ...
      for (int i = 0; i < nodes_el; i++) {
        conn[icount] = el_conn[nodesinv2[i]];  // el_map[0][el*nodes_el+i];
      ...
      for (int iside = 0; iside < el_sides; iside++) {
        if (el_neigh[iside] == -1) {  // domain boundary
          ...
          for (int idof = 0; idof < pt_sides_bd; idof++) {  // idof -> boundary node
            sur_toply[idof] =
                _GeomEl._surf_top(nodesinvbd2[idof] + pt_sides_bd * iside);  // use map to find global node
  ...
  mesh1->allocateCells(n_elements);
   if(_type_FEM[0]=27 && _dim==2)   for (int i = 0; i < n_elements; i++){ mesh1->insertNextCell(INTERP_KERNEL::NORM_QUAD9, nodes_el, conn + i * nodes_el); }
    if(_type_FEM[0]=27 && _dim==3)for (int i = 0; i < n_elements; i++){ mesh1->insertNextCell(INTERP_KERNEL::NORM_HEXA27, nodes_el, conn + i * nodes_el); }
    if(_type_FEM[0]=10  && _dim==2) for (int i = 0; i < n_elements; i++){ mesh1->insertNextCell(INTERP_KERNEL::NORM_TRI6 , nodes_el, conn + i * nodes_el); }
    if(_type_FEM[0]=10  && _dim==3) for (int i = 0; i < n_elements; i++){ mesh1->insertNextCell(INTERP_KERNEL::NORM_TETRA10, nodes_el, conn + i * nodes_el); }
  mesh1->finishInsertingCells();

  // MEDCouplingUMesh Mesh connectivity (boundary)
  MEDCoupling::MEDCouplingUMesh* mesh2 = MEDCoupling::MEDCouplingUMesh::New("Mesh_1", _dim - 1);
  mesh2->allocateCells(bd_n_elements);
 
    if(_type_FEM[0]=27 && _dim==2)   for (int i = 0; i < bd_n_elements; i++) {mesh2->insertNextCell(INTERP_KERNEL::NORM_SEG3, pt_sides_bd, conn_bd + i * pt_sides_bd); }
     if(_type_FEM[0]=27 && _dim==3) for (int i = 0; i < bd_n_elements; i++) {  mesh2->insertNextCell( INTERP_KERNEL::NORM_QUAD9, pt_sides_bd, conn_bd + i * pt_sides_bd);}
    if(_type_FEM[0]=10  && _dim==2)  for (int i = 0; i < bd_n_elements; i++) { mesh2->insertNextCell( INTERP_KERNEL::NORM_SEG3, pt_sides_bd, conn_bd + i * pt_sides_bd);}
       if(_type_FEM[0]=10  && _dim==3)  for (int i = 0; i < bd_n_elements; i++) { mesh2->insertNextCell(INTERP_KERNEL::NORM_TRI6, pt_sides_bd, conn_bd + i * pt_sides_bd);}
