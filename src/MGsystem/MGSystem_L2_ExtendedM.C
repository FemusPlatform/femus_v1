...
#ifdef HAVE_MED
#include "MGUtils_L1_InterfaceFunctionM.h"
...
#ifdef HAVE_MED
// =======================================================================
void EquationSystemsExtendedM::setBC(int name, int /*n_cmp*/, const MEDCouplingFieldDouble* field) {  
...
// =========================================================================
void EquationSystemsExtendedM::setBC(int name, int n_cmp, const char* s) {  
...
void EquationSystemsExtendedM::add_interface_fun(  ///< map position(return)
  ...
  int dim= _mg_mesh->_dim;
  // new function
  InterfaceFunctionM* fun = new InterfaceFunctionM(dim);
...
//   int Dim = fct->getSupport()->getSpaceDimension();
  ...
  MGSolDA* mgsyst = get_eqs(system_name);
  ...
  if (order ==
    ...
    for (unsigned i_mg = 0; i_mg < fct->getSupport()->getNumberOfCells(); i_mg++) {
...
//   int nNodes_med = support->getNumberOfNodes();
  ...
  // MGSolver -------------------------------------------------------
  MGSolDA* mgsyst = get_eqs(mgsystem_name.c_str());
  int Level = _mg_mesh->_NoLevels - 1;
  int offset = _mg_mesh->_NoNodes[Level];
  NumericVectorM* old_sol_top;
  if (store_value == 1)  old_sol_top = mgsyst->x_old[2][Level].get();
  else
    old_sol_top = mgsyst->x_old[0][Level].get();
  if (mgsystem_name == "NS2P") {
    old_sol_top = mgsyst->x_nonl[Level].get();  // MOD
  }
  // Mesh -----------------------------------------------------------
  ...
  for (int kvar = 0; kvar < n_cmp; kvar++) {
    ...
    if (order == 2) {  // --------------- order 2 -------------------------------------------------
      ...
      std::vector<mcIdType> conn;
      for (unsigned i_mg = 0; i_mg < fct->getSupport()->getNumberOfCells();
...
// ======================================================================
void EquationSystemsExtendedM::write_Boundary_value(std::string mgsystem_name, MEDCoupling::MEDCouplingFieldDouble* bcField) {  
...
void EquationSystemsExtendedM::write_Boundary_value(std::string mgsystem_name, MEDCoupling::MEDCouplingFieldDouble* bcField, std::map<int,int> map_bc2g) {  
  
  // MGSolver
  write_Boundary_value(mgsystem_name,bcField);
  MGSolDA* mgsyst = get_eqs(mgsystem_name.c_str());
  // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  // mgsyst->_map_bc2g = map_bc2g;
  // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  return;
}
// =======================================================================
MEDCoupling::MEDCouplingFieldDouble* EquationSystemsExtendedM::GetField(const std::string& systemName) {
  ...
  MGSolDA* mgsyst = get_eqs(systemName.c_str());
  MEDCoupling::MEDCouplingFieldDouble* ExtField = mgsyst->_ExtField->deepCopy();
  return ExtField;
}
// ======================================================================
const MEDCouplingUMesh* EquationSystemsExtendedM::getUMeshCoupling(int name) {  // get actual support
...
// =======================================================================
const MEDCouplingUMesh* EquationSystemsExtendedM::getUMeshCoupling_orig(int name) {  // get original support
