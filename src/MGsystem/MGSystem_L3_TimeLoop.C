    ...
    )                    // ========================================================================
{
  ...
  // restart -------------------------------------------
  if (restart) {
    double restartime = stod(_mgutils._dict_config["restartime"]);  // restart or not
...
{
  //  parameters
  int n_steps = stoi(_mgutils._dict_config["nsteps"]);
  double dt = stod(_mgutils._dict_config["dt"]);
  int print_step = stoi(_mgutils._dict_config["printstep"]);
...
// ===========================================================================
/// This function controls the transient loop
int MGTimeLoop::transient_onestep(
    const int& t_in,        ///< initial time iteration      (in)
    ...
    )  // ===================================================================================
{
  ...
  std::clock_t start_time = std::clock();
#endif  // -------------------------------------------------------------------
 int err= _mgeqmap.eqnmap_timestep_loop(time, t_step - t_in - 1, eq_min, eq_max);  // solve one step
#if PRINT_TIME == 1  // only for cpu time check ----------------------------------
  std::clock_t end_time = std::clock();
#endif  // ----------------------------------------------------------
  // B) print iteration t_step every print_step ********************************************
    ...
    )           // ========================================================================
{
  ...
  const int print_step = (int)(stoi(_mgutils._dict_config["printstep"]) + 0.5);
  const int nsteps = (int)(stoi(_mgutils._dict_config["nsteps"]) + 0.5);
...
// A Soving the system ****************************************************
  ...
  //   if(stoi(_mgutils._dict_config["FluidStructure"])!=0)     _mgeqmap.movemesh();
...
// ===============================================================================================
/// This function controls the transient loop by solving the system 
/// from  eq_min to  eq_max  (see femus/solvers/EquationsMap.C) 
double MGTimeLoop::transient_non_linear_solve(
    const int& t_in,        ///< initial time iteration      (in)
    const int& t_step,      ///< running time iteration      (in)
    const int& print_step,  ///< print every                 (in)
    double&    time,        ///< running time                (in)
...
{
// Solving the system from  eq_min to  eq_max  (see femus/solvers/EquationsMap.C) ***********
std::cout << "\n  ** Solving time step " << t_step << ", time = " << time << " ***" << std::endl;
TIME(std::clock_t start_time = std::clock();) // only for cpu time check --------------------------
double norm=  _mgeqmap.eqnmap_timestep_nonl_solve(time,t_step-t_in-1,toll,max_iter,eq_min,eq_max);// solve 
TIME(std::clock_t end_time = std::clock();)
TIME(  std::clock_t end_time2 = std::clock();
  std::cout << " Time solver ----->= " << double(end_time - start_time) / CLOCKS_PER_SEC
            << " Time printing ----->= " << double(end_time2 - end_time) / CLOCKS_PER_SEC << std::endl;)
  return norm;
}
...
// ===========================================================================
/// This function controls the transient loop
void MGTimeLoop::transient_update(
    const int& t_in,        ///< initial time iteration      (in)
    const int& t_step,      ///< running time iteration      (in)
    const int& print_step,  ///< print every                 (in)
    double& time,           ///< running time                (in)
...
TIME(  std::clock_t start_time = std::clock();)
  _mgeqmap.eqnmap_timestep_update(time, t_step - t_in - 1,toll, max_iter, eq_min, eq_max);  // solve one step
TIME(  std::clock_t end_time = std::clock();)
  // B) print iteration t_step every print_step ********************************************
  // print solution in xdmf format
  if (_mgeqmap._mgmesh._iproc == 0) {
...
TIME(  std::clock_t end_time2 = std::clock();
  std::cout << " Time update ----->= " << double(end_time - start_time) / CLOCKS_PER_SEC
            << " Time printing ----->= " << double(end_time2 - end_time) / CLOCKS_PER_SEC << std::endl;)
