...
MGUtils::MGUtils(
  ...
  _app_dir(app_dir),
  _myapp_name(app_name) {
  std::string suffix = (num_meshes == -1) ? "" : std::to_string(num_meshes);
  std::string loc_par_file = "/DATA/param_files_msh" + suffix + ".in";
  read(loc_par_file);

  if (_app_dir == "")  _app_dir = getenv("APP_PATH");
  _inout_dir = _app_dir + "/RESU/";
  _data_dir = _app_dir + "/DATA/";

  if (NUM_MESH > 1) { _data_dir = _app_dir + "/DATA/DATA" + std::to_string(num_meshes) + "/"; }
...
void MGUtils::read(const std::string& name_file_in) {
  ...
  if (_femus_dir == "") _femus_dir = getenv("FEMUS_DIR");
  if (_femus_dir == "") {
    std::cout << " Set FEMUS_DIR in your shell environment" << std::endl;
    abort();
  }

  if (_myapp_name == "") _myapp_name = getenv("FM_MYAPP");
  if (_myapp_name == "") {
