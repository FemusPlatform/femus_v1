...
void SalomeIO::read(const std::string& namefile  // namefile <-
  ...
  int dim_space=mesh.mesh_dimension();
  mesh.clear();
  ...
  int itype_vol = read_fem_type(dim_space,file_id, el_fem_type_vol, el_fem_type_bd);
  ...
  // reading xyz_med
  unsigned int n_nodes = dims[0] / dim_space;
...
void SalomeIO::read_bc_mat(
  ...
  // print bc and mat conditions into the file
  // "bc_case*.*"----------------------------- the file name
  char *data = getenv("APP_PATH");
  if (data) {
    std::string app_path = data;

    int num_bc_case = msh_count2 / 2 + 1;
    std::string num_bc = std::to_string(num_bc_case);
    std::string filename = "/RESU/bc_case.msh" + num_bc + ".h5";
    std::string bc_casedir = app_path + filename;
    msh_count2++;

    //  create a new file for storing material and boundary conditions flag
    hid_t file_id2 =
        H5Fcreate(bc_casedir.c_str(), H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
    hsize_t dimsf[2];
    // print bc_flag
    dimsf[0] = n_nodes;
    dimsf[1] = 1;
...
// ============================================================================
// This function reads the boundary condition and mat groups
int SalomeIO::read_fem_type(int dim,hid_t file_id, std::string& el_fem_type_vol, std::string& el_fem_type_bd) {
  ...
  if (dim == 3) {     // not valid in 3D as boundary
    fem_type_vol["QU4"] = 0;
    fem_type_vol["QU8"] = 0;
    fem_type_vol["QU9"] = 0;
    fem_type_vol["TR3"] = 0;
  ...
  if (dim == 2) {  // not valid in 3D as boundary
    fem_type_bd["TR6"] = 0;
    fem_type_bd["QU8"] = 0;
    fem_type_bd["QU9"] = 0;
    fem_type_bd["QU4"] = 0;
  ...
  if (dim == 1) {  // not valid in 3D as boundary
    fem_type_bd["SE2"] = 0;
    fem_type_bd["SE3"] = 0;
  }
  hid_t gid = H5Gopen(
